/*==============================================================================
 ...
 |              players' fleets consist of 3 ships that are hidden from the 
 |              enemy. Each ship may be differentiated by its "size"
 |              or number of cells it expands on the 
 |              game board. The Battleship has 4 cells, Cruiser has 3 cells,
...
int main (void)
{
	...
	WaterCraft ship[NUM_OF_SHIPS] = {{'b', 4, "Battleship"}, 
	                                 {'r', 3, "Cruiser"},
	                                 {'d', 2, "Destroyer"}};
	...
	short sunkShip[2][NUM_OF_SHIPS] = {{4, 3, 2},    
	                                   {4, 3, 2}};  /* tracks parts of the ship destroyed */
	...
	/**
	 * Both players' fleets consist of 3 ships that are hidden from the enemy. 
	 * Each ship may be differentiated by its "size" (besides the Battleship and 
	 * Destroyer) or number of cells it expands on the game board. 
	...
	/**
	 * Player1's ships are randomly placed.
	 */
	randomlyPlaceShipsOnGameBoard (playerOneGameBoard, ship);
	printf ("> Player 1 board has been generated.\n");

	/**
	 * Player2's ships are randomly placed. Note that the placement of 
	...
	while (TRUE) {
		...
		switch (player) {
			...
			case PLAYER_ONE: 
				...
				do {
					...
					if (target.row == 9 && target.column == 9)
					{
						stopGame = TRUE;
						break;
					}
		...
		// Patch: force getchar in case of player one (to fix non working getchar) 
